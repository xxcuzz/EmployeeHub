...
import { getGenderDisplay } from "../Models/Gender";
...
export function EmployeeItem({ employee, deleteEmployee, editEmployee, isSelected, toggleSelect, index }: EmployeeItemProps) {
  ...
  return (
    <li className={`flex items-center ${bgColor}`} key={employee.id}>
      ...
      <p className="flex-auto basis-md">{employee.age ? calculateAge(employee.age) + ' years' : '-'}</p>
      <p className="flex-auto basis-md">{getGenderDisplay(employee.gender)}</p>
